...
return Def.CourseContentsList {
	...
	SetCommand=function(self)
		...
		self:SetDestinationItem( math.max(0,self:GetNumItems() - 6) );
		self:SetLoop(false);
		self:SetMask(0,0);
	end;
...
-- 					self:diffuse( CustomDifficultyToColor(params.Difficulty) );
		...
		Def.TextBanner {
			InitCommand=cmd(x,-128;y,0;zoom,1;Load,"TextBannerCourse";SetFromString,"", "", "", "", "", "");
			SetSongCommand=function(self, params)
				if params.Song then
...
-- 					self:glow("1,1,1,0");
 		...
 		LoadFont("Common bold") .. {
			Text="0";
			InitCommand=cmd(x,180;y,0;zoom,0.75;shadowlength,1);
			SetSongCommand=function(self, params)
