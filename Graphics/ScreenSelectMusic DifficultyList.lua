	...
	Def.StepsDisplayList {
		...
		CursorP1 = Def.ActorFrame {
			...
			LoadActor(THEME:GetPathG("_StepsDisplayListRow","Cursor")) .. {
				InitCommand=cmd(diffuse,PlayerColor(PLAYER_1);x,12;zoom,0.5);
			};
			LoadActor(THEME:GetPathG("_StepsDisplayListRow","arrow")) .. {
				InitCommand=cmd(x,20;diffuse,PlayerColor(PLAYER_1);zoom,0.75;);
				OnCommand=cmd(thump,1;effectmagnitude,1,1.25,1;effectclock,'beat';);
			};
			LoadFont("Common Normal") .. {
				Text="P1";
				InitCommand=cmd(x,6;diffuse,PlayerColor(PLAYER_1);shadowlength,1);
		...
		CursorP2 = Def.ActorFrame {
			...
			LoadActor(THEME:GetPathG("_StepsDisplayListRow","Cursor")) .. {
				InitCommand=cmd(diffuse,PlayerColor(PLAYER_2);x,-12;zoom,0.5;zoomx,-0.5;);
			};
			LoadActor(THEME:GetPathG("_StepsDisplayListRow","arrow")) .. {
				InitCommand=cmd(x,-20;diffuse,PlayerColor(PLAYER_2);zoom,0.75;zoomx,-0.75);
				OnCommand=cmd(thump,1;effectmagnitude,1,1.25,1;effectclock,'beat';);
			};
			LoadFont("Common Normal") .. {
				Text="P2";
				InitCommand=cmd(x,-6;diffuse,PlayerColor(PLAYER_2);shadowlength,1);
				OnCommand=cmd(zoom,0.5);
			};
		};
