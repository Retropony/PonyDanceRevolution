...
--[[ 			Def.Quad {
			...
			LoadFont("_celestia redux 33px") .. {
				...
				InitCommand=cmd(y,-14;zoom,0.75;ztest,true);
			};
			LoadFont("Common normal") .. {
				InitCommand=cmd(y,8;zoom,0.6;vertspacing,-8;ztest,true;uppercase,true;);
				BeginCommand=function(self)
					local numSongsPlayed = profile:GetNumTotalSongsPlayed();
					local s = numSongsPlayed == 1 and tostring(THEME:GetString("ScreenSelectProfile","PSong")) or tostring(THEME:GetString("ScreenSelectProfile","PSongs"));
					local play = tostring(THEME:GetString("ScreenSelectProfile","Played"));
					-- todo: localize
					self:settext( numSongsPlayed.." "..s.." "..play );
				end;
			};
		};
		t[#t+1]=ProfileCard;
...
--[[ 	local t = LoadActor(THEME:GetPathB('', '_frame 3x3'), 'metal', 200, 230) .. {
	...
	t[#t+1] = Def.ActorFrame {
		Name = 'JoinFrame';
		LoadCard(GameColor.PonyMane[ ThemePrefs.Get("BestPony") ]);
--[[ 		Def.Quad {
		...
		LoadFont("_celestia redux 33px") .. {
			Text=tostring(THEME:GetString("ScreenSelectProfile","Join"));
	...
	t[#t+1] = Def.ActorFrame {
		...
		InitCommand=cmd(y,-2);
		Def.Quad {
			InitCommand=cmd(zoomto,180,40);
			OnCommand=cmd(diffuse,PlayerColor(Player);diffusealpha,0.7);
		};
	};
...
-- 			self:zoom(focus);
	...
	t[#t+1] = LoadFont("_celestia redux 33px") .. {
		...
		InitCommand=cmd(y,160;diffuse,color("#FFFFFF"););
	};
...
local t = Def.ActorFrame {
	...
	children = {
		Def.ActorFrame {
			Name = 'P1Frame';
			InitCommand=cmd(x,-120;y,SCREEN_CENTER_Y);
			OnCommand=cmd(decelerate,0.75;x,SCREEN_CENTER_X-160);
			OffCommand=cmd(accelerate,0.75;addy,-480);
		...
		Def.ActorFrame {
			Name = 'P2Frame';
			InitCommand=cmd(x,SCREEN_WIDTH+120;y,SCREEN_CENTER_Y);
			OnCommand=cmd(decelerate,0.75;x,SCREEN_CENTER_X+160);
			OffCommand=cmd(accelerate,0.75;addy,480);
			PlayerJoinedMessageCommand=function(self,param)
